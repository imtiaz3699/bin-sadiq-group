    ...
    return <div className="flex flex-row items-center text-white justify-center gap-20 w-full absolute py-6 px-10 z-10">
            {
                data.map((element,idx)=> {
                    ...
                    <div className="text-[18px] relative font-semi-bold cursor-pointer  flex flex-row items-center gap-2" onMouseEnter={idx === 4 ?  ()=> setDropDown(true) : ()=> setDropDown(false)} onMouseLeave={()=> setDropDown(false)}>
                    ...
                    {
                        element.linked && dropDown && idx === 4 && element.linked.map((item,index)=> {
                            return <>
